...
interface GraphQLResponse {
  data: {
    dojoStarterGameStatusEventModels: GameStatusEventModels;
  };
}
...
class GameStatusListenerImpl implements GameStatusListener {
  ...
  private async pollGameStatus(): Promise<void> {
    ...
    try {
      ...
      if (response.data && response.data.data) {
        this.processGameStatusData(response.data.data.dojoStarterGameStatusEventModels);
      } else {
  ...
  private processGameStatusData(gameStatusModels: GameStatusEventModels): void {
    ...
    currentStatuses.forEach(gameStatus => {
      const lastStatus = this.lastStatusMap.get(gameStatus.game_id);
      ...
      } else if (lastStatus !== gameStatus.status) {
        // Status changed
        console.log(`[GameStatusListener] Game ${gameStatus.game_id} status changed from ${lastStatus} to ${gameStatus.status}`);
        changedStatuses.push(gameStatus);
      }

      // Update last known status
      this.lastStatusMap.set(gameStatus.game_id, gameStatus.status);
    });

    // Log all current statuses
    console.log(`[GameStatusListener] Current games: ${currentStatuses.length}, Changed: ${changedStatuses.length}`);
    
    if (currentStatuses.length > 0) {
      console.log('[GameStatusListener] All current game statuses:', 
        currentStatuses.map(gs => `${gs.game_id}:${gs.status}`).join(', ')
      );
    }

    // Call callback only for changed statuses
  ...
  getLastKnownStatuses(): Map<string, string> {
